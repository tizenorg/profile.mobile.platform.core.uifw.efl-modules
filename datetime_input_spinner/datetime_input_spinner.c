...
_ampm_clicked_cb(void *data, Evas_Object *obj, void *event_info EINA_UNUSED)
{
   ...
   if ((curr_time.tm_hour >= 12) && (!strcmp(elm_object_text_get(obj), dgettext(PACKAGE, "AM"))))
     curr_time.tm_hour -= 12;
   else if ((curr_time.tm_hour < 12) && (!strcmp(elm_object_text_get(obj), dgettext(PACKAGE, "PM"))))
     curr_time.tm_hour += 12;
   else
     return;
...
{
   Input_Spinner_Module_Data *layout_mod;
   Evas_Object *field_obj, *inc_button, *dec_button, *text_button;
   layout_mod = (Input_Spinner_Module_Data *)module_data;
   ...
   if (field_type == ELM_DATETIME_AMPM)
     {
        ...
        elm_object_text_set(layout_mod->am_button, dgettext(PACKAGE, "AM"));
        ...
        elm_object_text_set(layout_mod->pm_button, dgettext(PACKAGE, "PM"));
   ...
   else
     {
        ...
        text_button = elm_object_part_content_get(field_obj, "elm.swallow.text_button");
        ...
        if (field_type == ELM_DATETIME_HOUR)
        ...
        else
          {
            elm_atspi_accessible_name_set(inc_button, dgettext(PACKAGE, "IDS_COM_BODY_INCREASE"));
             elm_atspi_accessible_description_set(dec_button, dgettext(PACKAGE, "IDS_COM_BODY_DECREASE"));
          }
